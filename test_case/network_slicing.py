...
def network_slicing(number_slices, total_number_centers, total_available_cpus, edges_adjacency_matrix,
    ...
    problem += (pulp.LpAffineExpression(
        ...
        [(Virtual_links[s, k, i, j], required_bandwidth[s, k]) for s in range(number_slices) for k
         in range(number_VNFs - 1) for i in range(edges_adjacency_matrix.shape[0]) for j in
