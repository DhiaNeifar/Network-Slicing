...
def flexible_network_slicing(number_slices, total_number_centers, total_available_cpus, edges_adjacency_matrix,
    ...
    VNFs_placements = np.array([[[pulp.LpVariable(f'slice_{s}_center_{c}_VNF_{k}', lowBound=0, upBound=1,
                                                  cat='Continuous') for c in range(total_number_centers)] for k in
                                 range(number_VNFs)] for s in range(number_slices)])
    ...
    '''
        ...
        [(unassigned_cpus[c], 1) for c in range(total_number_centers)]) +
                pulp.LpAffineExpression([(Virtual_links[s, k, i, j], required_bandwidth[s, k] + edges_delay[i, j])
                                         for s in range(number_slices)
                                         for k in range(number_VNFs - 1)
                                         for i in range(edges_adjacency_matrix.shape[0])
    ...
    problem += (pulp.LpAffineExpression(
        [(VNFs_placements[s, k, c], required_cpus[s, k]) for s in range(number_slices) for k in range(number_VNFs) for c
         in range(total_number_centers)]) - pulp.LpAffineExpression(
        [(Virtual_links[s, k, i, j], required_bandwidth[s, k]) for s in range(number_slices) for k in
         range(number_VNFs - 1) for i in range(edges_adjacency_matrix.shape[0]) for j in
         range(edges_adjacency_matrix.shape[1])]), 'Objective')
    ...
    VNFs_placements_check = np.array([[[pulp.LpVariable(f'slice_{s}_center_{c}_VNF_{k}_check', cat=pulp.LpBinary) for c
                                        in range(total_number_centers)] for k in range(number_VNFs)] for s in
                                      range(number_slices)])

    for s in range(number_slices):
        for k in range(number_VNFs):
            for c in range(total_number_centers):
                problem += (VNFs_placements[s, k, c] <= VNFs_placements_check[s, k, c], f'constraint {constraint}')
                constraint += 1

    for s in range(number_slices):
        for k in range(number_VNFs):
            problem += (pulp.LpAffineExpression([(VNFs_placements_check[s, k, c], 1)
                                                for c in range(total_number_centers)]) == 1, f'constraint {constraint}')
            constraint += 1
    ...
    for s in range(number_slices):
        for c in range(total_number_centers):
            problem += (pulp.LpAffineExpression([(VNFs_placements[s, k, c], 1) for k in range(number_VNFs)]) <= 1,
    ...
    for c in range(total_number_centers):
        problem += (pulp.LpAffineExpression(
            [(VNFs_placements[s, k, c], required_cpus[s, k]) for k in range(number_VNFs) for s in
             range(number_slices)]) <= total_available_cpus[c], f'constraint {constraint}')
        constraint += 1
    ...
    return (np.vectorize(pulp.value)(VNFs_placements), np.vectorize(pulp.value)(VNFs_placements_check),
            np.vectorize(pulp.value)(Virtual_links),
