...
def fairness_slicing(number_slices, total_number_centers, total_available_cpus, edges_adjacency_matrix,
    ...
    for c in range(total_number_centers):
        problem += (pulp.LpAffineExpression([(VNFs_placements[s, k, c], required_cpus[s, k])
                                             for s in range(number_slices)
                                             for k in range(number_VNFs)]) <=
                    total_available_cpus[c], f'constraint {constraint}')
        constraint += 1
    ...
    return status, (np.vectorize(pulp.value)(VNFs_placements), np.vectorize(pulp.value)(Virtual_links))
