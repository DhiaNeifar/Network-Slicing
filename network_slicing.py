...
def network_slicing(number_slices, total_number_centers, total_available_cpus, edges_adjacency_matrix,
    ...
    unassigned_cpus = np.array([pulp.LpVariable(f'unassigned_cpu_center_{c}', cat=pulp.LpInteger, lowBound=0) for c in
                                range(total_number_centers)])
    for c in failed_centers:
        unassigned_cpus[c] = pulp.LpVariable(f'unassigned_cpu_center_{c}', cat=pulp.LpInteger, lowBound=0, upBound=0)
    ...
    for c in range(total_number_centers):
        problem += pulp.lpSum(
            [(VNFs_placements[s, k, c] * required_cpus[s, k]) - unassigned_cpus[c] for k in range(number_VNFs) for
             s in range(number_slices)]) <= total_available_cpus[c], f'constraint {constraint}'
        constraint += 1
